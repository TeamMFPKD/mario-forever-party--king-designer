...
func update_drawing(current_pos: Vector2) -> void:
	...
	if points.size() >= 2:
		var prev = points[-2]
		var seg_dir = (last_point - prev).normalized()
		if seg_dir.dot(move_dir) < -0.9:
			is_reverse = true

	if is_reverse:
		# 回退：每反向移动 STEP 距离就删除一个点
		var axis_dist = abs(diff.x) if move_dir.x != 0 else abs(diff.y)
		if axis_dist >= STEP:
			var candidate = last_point + move_dir * STEP
			if points.size() >= 2 and candidate.distance_squared_to(points[-2]) < 0.1:
				points.pop_back()
				if points.size() > 0:
